...
const helpTypes: { value: HelpType; label: string }[] = [
  { value: 'small_hint', label: 'A nudge in the right direction' },
  { value: 'pattern_identification', label: 'Which pattern / technique to use' },
  { value: 'pseudocode', label: 'The approach spelled out in pseudocode' },
  { value: 'full_code', label: 'The full solution code' },
  { value: 'solution_video', label: 'A walkthrough video' }
];
...
export function AttemptForm({ problemId, onSaved }: { problemId: string; onSaved: () => void }) {
  ...
  return <form className="attempt-form" onSubmit={submit}><h2>Record attempt</h2>{error && <p role="alert">{error}</p>}<label>Date <input type="date" value={attemptedOn} onChange={(event) => setAttemptedOn(event.target.value)} required /></label><label>Outcome <select value={outcome} onChange={(event) => { setOutcome(event.target.value as Outcome); setHelpType(''); }}>{outcomes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>How hard did it feel? <select value={difficulty} onChange={(event) => setDifficulty(event.target.value as PerceivedDifficulty)}><option value="easy">Comfortable</option><option value="manageable">Manageable</option><option value="hard">A real struggle</option></select></label>{requiresHelp && <label>What did the help give you? <select value={helpType} onChange={(event) => setHelpType(event.target.value as HelpType)} required><option value="">Select what you looked up</option>{helpTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>}<label>Time spent (minutes, optional) <input type="number" min="1" max="600" step="1" value={duration} onChange={(event) => setDuration(event.target.value)} /></label><label>Notes for next time (optional) <textarea maxLength={500} value={notes} onChange={(event) => setNotes(event.target.value)} /></label><button type="submit">Save attempt</button></form>;
}
