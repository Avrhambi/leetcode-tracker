...
export function ProblemDetail({ problem, onBack }: { problem: CatalogProblem; onBack: () => void }) {
  ...
  return <section className="problem-detail"><button type="button" onClick={onBack}>Back to catalog</button><p className="eyebrow">{problem.primaryTopic}</p><h1>{problem.title}</h1><p>{problem.difficulty} · {problem.topics.join(', ')}</p><p><a href={problem.leetcodeUrl} target="_blank" rel="noreferrer">Solve on LeetCode</a></p>{progress && <p>Next review: {progress.nextReviewDate ?? 'Mastered'}</p>}{message && <p role="status">{message}</p>}<AttemptForm problemId={problem.id} onSaved={saved} /><h2>Attempt history</h2>{!attempts ? <p>Loading attempts…</p> : attempts.length === 0 ? <p>No attempts yet.</p> : <ol className="attempt-list">{attempts.map((attempt) => <li key={attempt.id}><strong>{attempt.attemptedOn}</strong> · {attempt.outcome.replaceAll('_', ' ')} · {attempt.quality}</li>)}</ol>}</section>;
}
