...
export default function App() {
  ...
  return <main><header className="app-header"><div className="app-intro"><p className="eyebrow">LeetCode Tracker / local training system</p><h1>LeetCode<br />Tracker</h1><p className="app-lede">A focused NeetCode 150 workbench for deliberate practice, reviews, and clear next steps.</p></div><nav className="terminal-nav" aria-label="Primary navigation"><span className="terminal-prompt" aria-hidden="true">$</span><button type="button" className={screen === 'dashboard' ? 'active' : ''} onClick={() => setScreen('dashboard')}>--dashboard</button><button type="button" className={screen === 'daily' ? 'active' : ''} onClick={() => setScreen('daily')}>--today</button><button type="button" className={screen === 'problems' ? 'active' : ''} onClick={() => setScreen('problems')}>--catalog</button><button type="button" className={screen === 'settings' ? 'active' : ''} onClick={() => setScreen('settings')}>--settings</button><span className="terminal-caret" aria-hidden="true">▮</span></nav></header>{screen === 'dashboard' ? <Dashboard attempts={attempts} problems={problems} progress={progress} /> : screen === 'daily' ? <DailyPlan items={plan ?? []} completedProblemIds={completedProblemIds} onComplete={(item) => completeRecommendation(item.problem.id, item.kind)} onSkip={(item) => skipRecommendation(item.problem.id, item.kind)} /> : screen === 'problems' ? <CatalogList problems={problems} progressByProblem={progressByProblem} onSelect={setSelectedProblem} /> : <Settings />}<footer className="app-footer"><p>LeetCode Tracker is local-first. Your practice history stays in this browser.</p></footer></main>;
}
